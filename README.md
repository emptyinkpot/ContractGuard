...
- 提交前门禁：`unified-gates/gates/pre-commit.mjs`
...
- gateway diagnose script：`C:\Users\ASUS-KL\.codex\automations\get-contractguard-gateway-status.ps1`
...
| 前端 closeout gate CLI | `guards/ai-behavior/core/check-frontend-design-closeout.mjs` |
| 前端 UI contract 模板 | `templates/project-contract/frontend-ui-contract.template.md` |
| 前端 closeout schema | `templates/project-contract/frontend-design-closeout.schema.json` |
...
- `templates/project-contract/frontend-ui-contract.template.md` 是最小可用前端 UI 契约模板。
- `templates/project-contract/frontend-design-closeout.schema.json` 是前端 closeout gate 输入 schema。
...
ContractGuard 现在内置一套可复用的极简前端治理能力，目标是用尽量低的流程复杂度挡住最常见的 UI 漂移问题。

最小链路如下：

1. 在 consumer repo 中为页面创建 `ui-contracts/*.md`
2. 使用 `templates/project-contract/frontend-ui-contract.template.md` 填写页面级契约
3. 由 CI 产出一个 closeout JSON，证明：
   - 组件来源来自白名单路径
   - 视觉回归通过
   - 可选 lint 结果已记录
4. 使用 `guards/ai-behavior/core/check-frontend-design-closeout.mjs` 校验 closeout JSON
...
轻量 lint 只做 advisory，不作为默认 block 条件。
...
& 'C:\Users\ASUS-KL\.codex\automations\get-contractguard-gateway-status.ps1' -Path 'E:\My Project\ContractGuard'
& 'C:\Users\ASUS-KL\.codex\automations\get-contractguard-gateway-status.ps1' -Path 'E:\My Project\ContractGuard' -AsTable
```
...
- 再做 `npm run selftest:decision-schema`，确认统一 decision 输出 schema 已接入。
- 再做 `npm run selftest:guard-regressions`，确认 docs-only diff 是 `allow`、real-risk diff 是 `block`。
- 如需从 hook 层直接验整链，执行 `powershell -File guards/ai-behavior/hooks/invoke-diff-gate.ps1 -ProjectRoot . -SelfTest`。
- 本地启用 git hook：`npm run hooks:install`
- 本地预提交门禁：`npm run gate:pre-commit`
- CI 门禁：`npm run gate:ci`
...
- `C:\Users\ASUS-KL\.codex\automations\get-contractguard-gateway-status.ps1` 负责输出当前实际 `profile / root / enabled / fallback` 解析结果，`-AsTable` 可切到精简摘要模式
