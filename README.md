...
- 对于未提供完整 repo-local gateway layout 的项目，`C:\\Users\\ASUS-KL\\.codex\\gateway.ps1` 会回退到 `E:\\My Project\\ContractGuard\\codex`。
...
- `full-executor`：完整执行通过 plan gate 的步骤。
...
- 模块：`context.mjs`、`compiler.mjs`、`planner.mjs`、`plan-gate.mjs`、`full-executor.mjs`、`diff-gate.mjs`、`safe-guard.mjs`、`actions/*`
- 默认输出：JSON 结果，包含 `goal`、`policyPath`、`compiledSteps`、`plannedSteps`、`planGate`、`steps`、`results`、`diffGate`、`trace`
- 目标：演示如何把高层目标先编译为结构化步骤，经 Plan Gate 细分后交给 Full Executor，再由 Diff Gate 检查偏离
...
- 在目标 repo 缺少完整 `codex/` gateway layout 时，为 Codex 提供稳定 fallback
